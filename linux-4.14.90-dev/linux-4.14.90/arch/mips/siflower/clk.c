...
static void __init sf16a18_cfg_setup(struct device_node *np, int cfg_type)
{
	unsigned char num_parents;
...
static void sf_pll_disable(struct clk_hw *hw)
{
	...
	/* pll should never be disabled */
	//WARN_ON(1);
	return;
#if 0
	if (hw->core->flags & CLK_IGNORE_UNUSED)
		return;
#endif
...
static void __init sf16a18_pll_setup(struct device_node *np, int pll_type)
{
	struct clk *clk;
